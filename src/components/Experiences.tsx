...
import { Clock, Users, Sunset, Star, Camera } from "lucide-react";
...
const Experiences = ({ referralCode }: { referralCode?: string }) => {
  ...
  const jetskiPacks = [
    {
      icon: Clock, name: t("exp_15min"), slug: "15-minutos", price: "50€", popular: false,
      includes: [t("exp_inc_1jetski"), t("exp_inc_lifejacket"), t("exp_inc_briefing"), t("exp_inc_insurance")],
    },
    {
      icon: Clock, name: t("exp_30min"), slug: "30-minutos", price: "80€", popular: true,
      includes: [t("exp_inc_1jetski"), t("exp_inc_lifejacket"), t("exp_inc_briefing"), t("exp_inc_insurance")],
    },
    {
      icon: Clock, name: t("exp_1hour"), slug: "1-hora", price: "120€", popular: false,
      includes: [t("exp_inc_1jetski"), t("exp_inc_lifejacket"), t("exp_inc_briefing"), t("exp_inc_insurance")],
    },
    {
      icon: Users, name: t("exp_group"), slug: "pack-grupo", price: "400€", popular: false,
      includes: [t("exp_inc_4jetski"), t("exp_inc_1hour"), t("exp_inc_guide"), t("exp_inc_briefing"), t("exp_inc_insurance")],
    },
  ];
  ...
  return (
    <section id="experiencias" className="relative bg-foam">
      <WaveDivider bgColor="hsl(190 30% 96%)" flip className="absolute top-0 left-0 right-0 z-10 -translate-y-full" />
          ...
          <AnimatePresence mode="wait">
            {activeTab === "barcos" ? (
              ...
              <motion.div
                ...
                exit={{ opacity: 0, y: -20 }}
                transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
                className={`grid grid-cols-1 sm:grid-cols-2 ${currentPacks.length > 2 ? "lg:grid-cols-4" : "lg:grid-cols-2 max-w-2xl mx-auto"} gap-6`}
              >
                {currentPacks.map((pkg, i) => (
                  ...
                  >
                    ...
                    <h3 className="font-display font-700 text-xl text-foreground mb-1">{pkg.name}</h3>
                    <div className="mb-1">
                      <span className="font-display text-3xl font-800 text-foreground">{pkg.price}</span>
                    </div>
                    {activeTab === "jetski" && (
                      <p className="text-xs text-muted-foreground mb-5">{t("exp_max_2")}</p>
                    )}
                    ...
                    <motion.button
                      onClick={() => navigate(`/reservar?pack=${pkg.slug}${referralCode ? `&ref=${referralCode}` : ""}`)}
