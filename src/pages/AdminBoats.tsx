             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {formData.extraOptions?.map((opt, idx) => (
                   <div key={idx} className="bg-background p-6 rounded-2xl border border-border/50 shadow-md relative group space-y-6 transition-all hover:shadow-lg">
                      ...
                      <div className="space-y-4">
                         <Input placeholder="Detalhe + Enter..." className="h-10 text-sm bg-muted/10 border-dashed" onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); const val = e.currentTarget.value.trim(); if (!val) return; e.currentTarget.value = ''; setFormData(p => { const o = (p.extraOptions || []).map((item, i) => i === idx ? { ...item, details: [...(item.details || []), val] } : item); return { ...p, extraOptions: o }; }); } }} />
                         <div className="flex flex-wrap gap-2">{opt.details?.map((d, di) => <span key={di} className="bg-muted px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 border border-border/40 shrink-0">{d}<button type="button" onClick={() => setFormData(p => { const o = [...(formData.extraOptions || [])]; o[idx].details = o[idx].details?.filter((_, idx2) => idx2 !== di); return { ...p, extraOptions: o }; })} className="hover:text-destructive"><X size={12}/></button></span>)}</div>
                      </div>
                   </div>
                ))}
             </div>
