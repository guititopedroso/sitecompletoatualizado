             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {formData.extraOptions?.map((opt, idx) => (
                   <div key={idx} className="bg-background p-8 rounded-3xl border border-border/50 shadow-xl relative group space-y-6 transition-all hover:scale-[1.01]">
                      ...
                      <div className="space-y-4">
                         <Input placeholder="+ Adicionar Detalhe (Enter)" className="h-11 text-sm bg-muted/10 border-dashed" onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); const val = e.currentTarget.value.trim(); if (!val) return; e.currentTarget.value = ''; setFormData(p => { const o = (p.extraOptions || []).map((item, i) => i === idx ? { ...item, details: [...(item.details || []), val] } : item); return { ...p, extraOptions: o }; }); } }} />
