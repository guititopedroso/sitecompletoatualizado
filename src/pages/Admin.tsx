...
import { Lock, ChevronLeft, ChevronRight, Plus, Trash2, CalendarDays, LogOut, Loader2, MapPin, Phone, Mail, Users, Clock, Package, ChevronDown, Camera, UserCircle, Link2, Gift, Trophy, CreditCard, CheckCircle2, ImageIcon } from "lucide-react";
...
const Admin = () => {
  ...
  const [activePanel, setActivePanel] = useState("bookings"); // bookings, referrals, gallery
  ...
  const togglePanel = (panel: string) => {
    setActivePanel(activePanel === panel ? "bookings" : panel);
    if(panel === 'referrals' && activePanel !== 'referrals') fetchReferralStats();
  };
      ...
      <div className="ocean-gradient py-4 px-4 sm:px-6 shadow-ocean">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Button
              ...
              className={`text-primary-foreground/70 hover:text-primary-foreground hover:bg-primary-foreground/10 ${activePanel === 'gallery' ? 'bg-primary-foreground/10' : ''}`}>
              <ImageIcon size={16} />
              <span className="hidden sm:inline ml-1">Galeria</span>
            </Button>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => togglePanel("referrals")}
              className={`text-primary-foreground/70 hover:text-primary-foreground hover:bg-primary-foreground/10 ${activePanel === 'referrals' ? 'bg-primary-foreground/10' : ''}`}>
              <Link2 size={16} />
