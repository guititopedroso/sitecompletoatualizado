...
const Profile = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD]">
      {/* Dynamic Header */}
      <div className="ocean-gradient pt-16 sm:pt-24 pb-24 sm:pb-32 relative overflow-hidden">
           ...
           <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 sm:gap-8">
              <div className="flex items-center gap-4 sm:gap-6">
                 <div className="relative group shrink-0">
                    <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl sm:rounded-[2rem] bg-white p-1 shadow-2xl relative overflow-hidden flex items-center justify-center">
                       {photoLoading ? (
                       ...
                       {user?.photoURL ? (
                         <img 
                           src={user.photoURL} 
                           alt="Profile" 
                           className="w-full h-full rounded-xl sm:rounded-[1.8rem] object-cover"
                           onError={(e) => {
                       ...
                       <div className={cn("fallback-icon items-center justify-center w-full h-full bg-muted/50 rounded-xl sm:rounded-[1.8rem]", user?.photoURL ? "hidden" : "flex")}>
                          <User size={32} className="text-muted-foreground/40 sm:w-10 sm:h-10" />
                       </div>
                    ...
                    <div className="absolute -bottom-1 -right-1 sm:-bottom-2 sm:-right-2 w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-secondary text-white flex items-center justify-center border-2 sm:border-4 border-[#1e4b6e] shadow-lg z-20">
                       <Zap size={14} className="sm:w-[18px] sm:h-[18px]" fill="currentColor" />
                    </div>
                 </div>
                 <div className="min-w-0">
                    <h1 className="font-display text-2xl sm:text-3xl md:text-4xl font-900 text-white tracking-tight leading-tight mb-1 sm:mb-2 truncate">
                       Olá, {firstName || user?.displayName?.split(" ")[0] || "Explorador"}!
                    </h1>
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                       <span className="bg-white/10 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-900 text-white uppercase tracking-widest border border-white/10 truncate max-w-[180px] sm:max-w-none">
                          {user?.email}
                       </span>
                       <span className="w-1.5 h-1.5 rounded-full bg-secondary animate-pulse hidden sm:inline-block" />
                       <span className="text-white/70 text-[10px] sm:text-xs font-semibold">Cliente Royal Coast</span>
                    </div>
                 </div>
              </div>
              ...
              <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                 <Button 
                    onClick={() => navigate("/")}
                    variant="outline" 
                    className="flex-1 sm:flex-initial bg-white/10 border-white/20 text-white hover:bg-white/20 rounded-xl sm:rounded-2xl h-10 sm:h-12 px-4 sm:px-6 text-xs sm:text-sm font-bold"
                 >
                    Home
                 </Button>
                 <Button 
                    onClick={handleLogout}
                    className="flex-1 sm:flex-initial bg-coral hover:bg-coral/90 text-white rounded-xl sm:rounded-2xl h-10 sm:h-12 px-4 sm:px-6 text-xs sm:text-sm font-bold shadow-lg shadow-coral/20 border-none"
                 >
                    <LogOut size={16} className="mr-1.5 sm:mr-2" />
                    Sair
                 </Button>
              </div>
           </div>
      ...
      <div className="max-w-5xl mx-auto px-3 sm:px-4 -translate-y-12 sm:-translate-y-16">
         {/* Tabs Navigation */}
         <div className="bg-white rounded-2xl sm:rounded-[2rem] shadow-2xl shadow-black/5 p-1.5 sm:p-2 mb-6 sm:mb-8 border border-border/40 flex gap-1.5 sm:gap-2">
            <button
               onClick={() => setActiveTab("active")}
               className={cn(
                  "flex-1 py-3 sm:py-4 px-2 sm:px-6 rounded-xl sm:rounded-[1.5rem] font-display font-900 text-[10px] sm:text-xs uppercase tracking-wider sm:tracking-[0.15em] transition-all flex items-center justify-center gap-1.5 sm:gap-3",
                  activeTab === "active" 
                     ? "bg-primary text-white shadow-xl shadow-primary/20" 
                     : "text-muted-foreground hover:bg-muted/50"
               )}
            >
               <LayoutDashboard size={16} className="shrink-0 sm:w-[18px] sm:h-[18px]" />
               Reservas
            </button>
            <button
            ...
            >
               <Archive size={16} className="shrink-0 sm:w-[18px] sm:h-[18px]" />
               Histórico
            </button>
            <button
            ...
            <AnimatePresence mode="wait">
               {activeTab === "settings" ? (
                  <motion.div
                    ...
                    className="bg-white rounded-2xl sm:rounded-[2.5rem] shadow-xl shadow-black/5 border border-border/40 p-5 sm:p-8 md:p-12 max-w-2xl mx-auto"
                 >
                    <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-10 pb-4 sm:pb-6 border-b border-border/50">
                       ...
                       <div>
                          <h2 className="font-display text-xl sm:text-2xl font-800 text-foreground">Definições da Conta</h2>
                          <p className="text-xs sm:text-sm text-muted-foreground font-medium uppercase tracking-wider mt-0.5 sm:mt-1">Gere o teu perfil e segurança</p>
                       </div>
                    </div>
                    ...
                    <form onSubmit={handleUpdateProfile} className="space-y-6 sm:space-y-8">
                             ...
                             <div className="space-y-2 sm:space-y-3">
                                ...
                                <div className="relative">
                                   ...
                                   <Input 
                                      ...
                                      className="h-12 sm:h-14 pl-12 rounded-xl sm:rounded-2xl border-border/50 bg-muted/20 focus-visible:ring-primary/20 font-semibold text-sm"
                                      placeholder="Ex: João"
                                   />
                                </div>
                             </div>
                             ...
                             <div className="space-y-2 sm:space-y-3">
                                ...
                                <div className="relative">
                                   ...
                                   <Input 
                                      ...
                                      className="h-12 sm:h-14 pl-12 rounded-xl sm:rounded-2xl border-border/50 bg-muted/20 focus-visible:ring-primary/20 text-sm"
                                   />
                                </div>
                             </div>
                             <div className="space-y-2 sm:space-y-3">
                                ...
                                <div className="flex gap-2">
                                   <select 
                                      ...
                                      className="w-[95px] sm:w-[100px] h-12 sm:h-14 rounded-xl sm:rounded-2xl border border-border/50 bg-muted/20 text-foreground text-xs sm:text-sm font-semibold focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all px-2.5 appearance-none cursor-pointer"
                                   >
                          ...
                          <div className="space-y-2 sm:space-y-3">
                             ...
                             <div className="relative opacity-60">
                                ...
                                <Input 
                                   value={user?.email || ""}
                                   disabled
                                   className="h-12 sm:h-14 pl-12 rounded-xl sm:rounded-2xl bg-muted/40 cursor-not-allowed border-border/50 text-sm"
                                />
                             </div>
                          </div>
                       ...
                       <Button 
                          ...
                          className="w-full h-12 sm:h-14 sunset-gradient text-white rounded-xl sm:rounded-2xl font-display font-900 uppercase tracking-widest text-xs shadow-xl shadow-coral/20 hover:scale-[1.01] transition-all"
                       >
                          {settingsLoading ? <Loader2 className="animate-spin" /> : "Guardar Alterações"}
                       </Button>
                    </form>

                    <div className="mt-8 sm:mt-12 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-secondary/5 border border-secondary/10 flex items-start gap-3 sm:gap-4">
                       <CheckCircle2 size={20} className="text-secondary shrink-0 sm:w-6 sm:h-6 mt-0.5" />
                       <div>
                          <h4 className="font-display font-800 text-xs sm:text-sm text-foreground">Conta Segura</h4>
                          <p className="text-[11px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 leading-relaxed">
                    ...
                    <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-border/50">
                       <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 text-destructive">
                          <AlertTriangle size={16} className="sm:w-5 sm:h-5" />
                          <h4 className="font-display font-800 text-xs sm:text-sm uppercase tracking-widest">Zona de Perigo</h4>
                       </div>
                       <div className="bg-destructive/5 border border-destructive/10 rounded-2xl sm:rounded-3xl p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group hover:bg-destructive/10 transition-colors">
                          <div className="flex-1">
                             <p className="text-xs font-bold text-foreground">Excluir Conta Permanentemente</p>
                             <p className="text-[10px] text-muted-foreground mt-1 leading-relaxed">
                                Ao excluir a conta, todos os teus dados, reservas e histórico serão eliminados.
                             </p>
                          </div>
                          <Button 
                             onClick={handleDeleteAccount}
                             variant="outline" 
                             className="w-full sm:w-auto border-destructive/20 text-destructive hover:bg-destructive hover:text-white rounded-xl h-10 px-4 text-[10px] font-900 uppercase tracking-widest transition-all"
                          >
                  ...
                  >
                     {(activeTab === "active" ? activeBookings : pastBookings).length > 0 ? (
                        (activeTab === "active" ? activeBookings : pastBookings).map((booking) => (
                           <div 
                              key={booking.id}
                              className="bg-white rounded-2xl sm:rounded-[2.5rem] shadow-xl shadow-black/5 border border-border/40 overflow-hidden group hover:border-primary/30 transition-all hover:scale-[1.01]"
                           >
                              <div className="bg-muted/30 p-4 sm:p-6 flex flex-wrap sm:flex-nowrap items-center justify-between border-b border-border/40 gap-2">
                                 <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                       <p className="text-[9px] sm:text-[10px] font-900 text-muted-foreground uppercase tracking-widest">{booking.id.slice(0, 8)}</p>
                                       <h3 className="font-display font-800 text-foreground text-sm sm:text-base truncate max-w-[140px] sm:max-w-[180px]">{booking.pack_name}</h3>
                                    </div>
                                 </div>
                                 <div className="text-right shrink-0">
                                    <p className="text-base sm:text-lg font-display font-900 text-primary">{booking.price}€</p>
                                    <span className={cn(
                                       "inline-block px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-900 uppercase tracking-widest",
                              ...
                              <div className="p-4 sm:p-8 grid grid-cols-2 gap-y-4 gap-x-4 sm:gap-y-6 sm:gap-x-8">
                                 <div className="space-y-0.5 sm:space-y-1">
                                    <div className="flex items-center gap-1.5 text-[9px] sm:text-[10px] font-800 uppercase tracking-widest text-muted-foreground">
                                       <Calendar size={12} className="text-primary shrink-0" />
                                       Data
                                    </div>
                                    <p className="font-display font-900 text-foreground text-xs sm:text-sm uppercase">
                                       {format(parseISO(booking.booking_date), "dd MMM yyyy", { locale: pt })}
                                    </p>
                                 </div>
                                 ...
                                 <div className="space-y-0.5 sm:space-y-1">
                                    ...
                                    <p className="font-display font-900 text-foreground text-xs sm:text-sm">{booking.booking_time}</p>
                                 </div>
                                 ...
                                 <div className="space-y-0.5 sm:space-y-1">
                                    ...
                                    <p className="font-display font-900 text-foreground text-xs sm:text-sm truncate uppercase">{booking.location}</p>
                                 </div>
                                 ...
                                 <div className="space-y-0.5 sm:space-y-1">
                                    ...
                                    <p className="font-display font-900 text-foreground text-xs sm:text-sm uppercase">{booking.num_people} Pessoas</p>
                                 </div>
                              </div>

                              <div className="px-4 sm:px-8 pb-4 sm:pb-8 flex flex-col sm:flex-row items-stretch sm:items-center justify-between border-t border-border/40 pt-3 sm:pt-4 gap-2 sm:gap-3">
                                 <div className="flex items-center justify-center sm:justify-start gap-2 bg-primary/5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl border border-primary/10">
                                    <ShieldCheck size={14} className="text-primary shrink-0" />
                                    <span className="text-[9px] sm:text-[10px] font-900 text-primary uppercase tracking-widest">Reserva Validada</span>
                                 </div>
                                 {activeTab === "active" && (
                                    <Button
                                       variant="ghost"
                                       onClick={() => setCancelModalBooking(booking)}
                                       className="text-destructive hover:bg-destructive/10 text-[11px] sm:text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-1.5 rounded-xl h-9 sm:h-10 px-3 w-full sm:w-auto"
                                    >
                        ...
                        <div className="bg-white rounded-2xl sm:rounded-[2.5rem] shadow-xl shadow-black/5 border border-border/40 p-8 sm:p-16 text-center col-span-1 md:col-span-2">
                           <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl sm:rounded-3xl bg-muted/30 flex items-center justify-center mx-auto mb-4 sm:mb-6 text-muted-foreground">
                              {activeTab === "active" ? <Zap size={28} className="sm:w-8 sm:h-8" /> : <Archive size={28} className="sm:w-8 sm:h-8" />}
                           </div>
                           <h3 className="font-display text-xl sm:text-2xl font-800 text-foreground mb-2">
                              {activeTab === "active" ? "Não tens reservas ativas" : "O teu histórico está vazio"}
                           </h3>
                           <p className="text-xs sm:text-sm text-muted-foreground max-w-xs mx-auto mb-6 sm:mb-8 font-medium">
                              Parece que ainda não tens nenhuma viagem marcada. Estás pronto para a tua próxima aventura?
                           </p>
                           <Button 
                              onClick={() => navigate("/#experiencias")}
                              className="sunset-gradient text-white rounded-xl sm:rounded-2xl h-12 sm:h-14 px-8 sm:px-10 font-display font-900 uppercase tracking-widest text-xs w-full sm:w-auto"
                           >
                              Reservar Agora
                           </Button>
                        </div>
